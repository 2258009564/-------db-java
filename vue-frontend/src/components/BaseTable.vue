      ...
      >
        ...
        >
          <template #default="scope">
            ...
            <el-tag
              v-else-if="col.type === 'stock'"
              :type="scope.row[col.prop] < 10 ? 'danger' : 'success'"
            >
              {{ scope.row[col.prop] }}
            </el-tag>
  ...
  fruits: {
    ...
    fields: [
      { prop: "name", label: "名称" },
      {
        prop: "price",
        label: "单价",
        // type removed to use el-input for custom validation
        rules: [
          { required: true, message: "请输入单价", trigger: "blur" },
          { pattern: /^\d+$/, message: "单价必须为整数", trigger: "blur" },
        ],
      },
      { prop: "unit", label: "单位" },
...
const submitForm = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        if (isEdit.value) {
          await request.put(`/${props.type}`, formData.value);
        } else {
